...
buttons.forEach((button) => {button.addEventListener('click', () => {
    ...
    computerChoiceNum = computerPlay(compChoice);
    playGame();
    })
})
...
function playRound(){
    let win_array = [[0, 2, 1],
                    [1, 0, 2],
                    [2, 1, 0]];
    let result = win_array[playerChoiceNum][compChoiceNum];
        if (result == 0){
...
function playGame() {
    ...
    computer.textContent = `Computer Score: ${compScore}`;
    if (playerScore == 5) {
        output.textContent = "Yayyyy You Won!!";
        playerScore = 0;
        compScore = 0;
        player.textContent = `Player Score: ${playerScore}`;
        computer.textContent = `Computer Score: ${compScore}`;
    } else if (compScore == 5){
        output.textContent = "You Lost the game:/ You are not wiser than the computer";
        playerScore = 0;
        compScore = 0;
        player.textContent = `Player Score: ${playerScore}`;
        computer.textContent = `Computer Score: ${compScore}`;
